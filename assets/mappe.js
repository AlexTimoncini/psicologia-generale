...
const MP_ICONA_SCHERMO = '<svg viewBox="0 0 16 16" width="14" height="14" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M1.5 5.5v-4h4M10.5 1.5h4v4M14.5 10.5v4h-4M5.5 14.5h-4v-4"/></svg>';
const MP_ICONA_ESCI = '<svg viewBox="0 0 16 16" width="14" height="14" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M5.5 1.5v4h-4M14.5 5.5h-4v-4M10.5 14.5v-4h4M1.5 10.5h4v4"/></svg>';
...
function vistaMappe(idMacro) {
  ...
  main.innerHTML = `
    ...
    <div class="mp-banco" id="mpBanco">
      <div class="mp-scena${st.vista === 'mappa' ? '' : ' mp-nascosta'}" id="mpScena" tabindex="-1">
        ...
        <div class="mp-zoom">
          ${puoSchermoIntero ? `<button id="mpSchermo" aria-label="Schermo intero" title="Schermo intero">${MP_ICONA_SCHERMO}</button>` : ''}
  ...
  if (btnSchermo) {
    const suFullscreen = () => {
      const attivo = document.fullscreenElement === scena;
      const etichetta = attivo ? 'Esci da schermo intero' : 'Schermo intero';
      btnSchermo.setAttribute('aria-label', etichetta);
      btnSchermo.title = etichetta;
      btnSchermo.innerHTML = attivo ? MP_ICONA_ESCI : MP_ICONA_SCHERMO;
    ...
    btnSchermo.onclick = () => {
      ...
      else scena.requestFullscreen().catch(() => {});
    };
  }
